...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function initLanguageSlider() {
    function switchToSlide(targetId) {
      ...
      const targetSlide = document.getElementById(cleanId);
      const allSlides = document.querySelectorAll('.lang-slide');

      if (targetSlide && allSlides.length > 0) {
        allSlides.forEach(slide => slide.classList.remove('active'));
        targetSlide.classList.add('active');
      }
    }

    // A. Check URL Hash on page load
    const initialHash = window.location.hash || '#japanese';
    switchToSlide(initialHash);

    // B. Handle clicks on dropdown links dynamically
    ...
    langLinks.forEach(link => {
      link.addEventListener('click', () => {
        ...
        if (href.includes('#')) {
          ...
          if (targetId) {
            switchToSlide(targetId);
          }
        }
      });
    });

    // C. Fallback listener for URL hash changes
    window.addEventListener('hashchange', () => {
      switchToSlide(window.location.hash);
    });
  })();
...
//   // Function to switch active slide by ID
//   function switchToSlide(targetId) {
//     if (!targetId) return;
...
//       // 2. Add active class ONLY to target slide
//       targetSlide.classList.add('active');
//     }
//   }
...
//   // B. Handle clicks on language dropdown links explicitly
//   const langLinks = document.querySelectorAll('a[href*="#"]');
//   langLinks.forEach(link => {
